...
describe("list_repository_members", () => {
  ...
  it("authenticates with the agent key when that is the only key set", async () => {
    // The members endpoints answer BOTH key kinds since SPGD-952 (`8a30cba`) — the
    // set reachable is bounded by whichever was presented — so an agent holding
    // only the agent credential can read a repository's members where its grant
    // carries `members.manage`. Nothing on the wire changes beyond the Bearer:
    // same GET, same path.
    const http = stubFetch({ status: 200, body: BODY });
